...
En esta sección se inspecciona visualmente la evolución de la matrícula para identificar la presencia de tendencia, saltos inusuales y cambios de variabilidad. Estas observaciones guiarán las transformaciones y el modelado posteriores.
...
Para ello se combinan dos pruebas complementarias de raíz unitaria —**ADF** y **KPSS**— y la prueba **ARCH** de Engle; además se estima el parámetro de **Box–Cox** (λ) como indicador de dependencia de la varianza con el nivel.
...
A la luz de los contrastes estadísticos mostrados en la Tabla&nbsp;1, la serie en nivel **no satisface** los supuestos básicos de estacionariedad ni de varianza constante:
...
En síntesis, la serie original **presenta raíz unitaria** y **varianza no constante**.   Tener una raíz unitaria significa que la serie se comporta como un paseo aleatorio: cada perturbación desplaza permanentemente su nivel, la varianza se expande con el tiempo y la media ya no permanece fija.
...
**heterocedasticidad**.  Trabajar con una serie que viole estos supuestos puede llevar a
estimaciones sesgadas y predicciones poco fiables; por lo tanto, es imprescindible aplicar
una transformación que devuelva la media y la varianza a un comportamiento estable a lo
largo del tiempo.
...
La combinación *ADF no rechaza* + *KPSS rechaza* confirma la existencia de una **raíz unitaria**,
propia de un proceso tipo “paseo aleatorio”: los choques no se disipan, la varianza crece con
el tiempo y la media no es estable.  
Al mismo tiempo, el test ARCH muestra que la varianza cambia de forma sistemática y el valor
...
es aplicar **una diferencia de primer orden**.  
La diferenciación elimina la raíz unitaria (resta sucesiva \(Y_t - Y_{t-1}\)) y, en la práctica,
suele amortiguar los cambios de varianza en series anuales como la presente.  
En la próxima subsección se comprueba empíricamente si esa única diferencia basta o si es
...
Tras la primera diferencia, el ADF rechaza la raíz unitaria y el KPSS arroja un p-valor de 0,0488; el test ARCH no detecta heterocedasticidad. Visualmente, la serie diferenciada oscila de forma estable alrededor de su constante, con picos pasajeros que regresan rápidamente a la media. Estos resultados confirman que una sola diferencia es suficiente. aplicar una segunda sólo añadiría complejidad sin mejorar la estacionariedad ni la varianza.
...
Ambas gráficas se construyen a partir de la serie ya diferenciada (ΔY), la cual hemos comprobado que es estacionaria tanto en media como en varianza, según las pruebas previas y el análisis visual.
...
La PACF muestra la relación directa entre los valores de la serie y sus retardos, eliminando el efecto de los retardos intermedios. En la gráfica:
...
Además de las pruebas estadísticas, se presentan dos gráficos para complementar la interpretación:
...
- El **gráfico Q–Q (quantile–quantile)** compara los cuantiles teóricos de una distribución normal estándar con los cuantiles empíricos de los residuos. En este gráfico, la mayoría de los puntos se alinean con la diagonal roja, lo cual indica que **la distribución de los residuos es aproximadamente normal**, aunque se observa una ligera asimetría en los valores extremos, considerada manejable.
